    ...
    >
      ...
      <div className="w-[65%] md:w-[60%] p-3 md:p-4 flex flex-col justify-center relative">
        ...
        >
          {/* Tech Stack Icons */}
          {techIcons.length > 0 && (
            <div className="w-[60%] md:w-[30%] relative bg-gray-900 rounded-lg p-2 border border-gray-700 group-hover:border-gray-600 transition-colors duration-300">
              
              {/* Mobile: Infinite Scroll */}
              <div className="block md:hidden overflow-hidden w-full">
                <div className="flex items-center space-x-4 animate-scroll whitespace-nowrap">
                  {techIcons.concat(techIcons).map((IconComponent, idx) => (
                    <div
                      key={idx}
                      className="text-lg flex-shrink-0 hover:scale-110 transition-all duration-200"
                    >
                      {IconComponent}
                    </div>
                  ))}
          ...
          <div className="flex gap-2">
            {demoLink && (
              <a
                ...
                className={`
                  flex items-center justify-center w-7 h-7 md:w-8 md:h-8 bg-blue-600 hover:bg-blue-700 
                ...
                title="Live Demo"
              >
                <ExternalLink className="text-xs md:text-sm text-white group-hover/btn:scale-110 transition-transform" />
              </a>
            )}
            {githubLink && (
              <a
                ...
                rel="noopener noreferrer"
                className={`
                  flex items-center justify-center w-7 h-7 md:w-8 md:h-8 bg-gray-700 hover:bg-gray-600 
                  rounded-full transition-all duration-300 group/btn
                  transform
                  ${isVisible ? 'scale-100 rotate-0' : 'scale-0 rotate-180'}
              ...
              >
                <Github className="text-xs md:text-sm text-white group-hover/btn:scale-110 transition-transform" />
              </a>
            )}
          </div>
        </div>
